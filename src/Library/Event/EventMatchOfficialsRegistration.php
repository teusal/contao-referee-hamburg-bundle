...
class EventMatchOfficialsRegistration extends AbstractEventParticipiantHandler
{
    ...
    {
        if (Input::get('key') != 'spiele') {
            return '';
        }

        // register match officials
        if (Input::post('FORM_SUBMIT') == 'match_officials_registration') {
            ...
            if(empty($arrToSave)) {
                ...
                foreach ($arrToSave as $toSave) {
                    ...
                    if ($exist->next()) {
                        ...
                        $insert = $this->Database->prepare("INSERT INTO tl_bsa_teilnehmer (pid, tstamp, sr_id, sr, typ) SELECT ?, ?, id, name_rev, 's' FROM tl_bsa_schiedsrichter WHERE id = ?")
        ...
        $endDate = strtotime('+1 DAY', intval($startDate));
